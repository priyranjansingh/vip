...
class InvoicePayment extends Am_Record {
    ...
    public function setFromTransaction(Invoice $invoice, Am_Paysystem_Transaction_Abstract $transaction)
    {
        ...
        if ($amount<=0)
            $amount = ($isFirst && doubleval($invoice->first_total)) ? $invoice->first_total : $invoice->second_total;
...
class InvoicePaymentTable extends Am_Table {
    ...
    {
        return $this->selectObjects("SELECT ip.*,
            ...
            FROM ?_invoice_payment ip LEFT JOIN ?_user u USING (user_id)
