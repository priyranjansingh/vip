...
', $level) : ___('Global Commission')) : $types[$this->type];
    ...
    function renderConditions()
    {
        ...
        foreach ($this->getConditions() as $conditionType => $vars)
        {
            ...
            {
                case self::COND_AFF_ITEMS_COUNT:;
                    $ret[] = sprintf("affiliate generated %d item sales last %d days",
                ...
                case self::COND_COUPON:
                    ...
                    switch ($vars['type']) {
                            case 'any' :
                                ...
                                break;
                            case 'coupon' :
                                $end = 'Coupon with Code : ' . $vars['code'];
                                break;
                            case 'batch' :
                                $options = $this->getDi()->couponBatchTable->getOptions(false);
                                $end = 'Coupon from Batch : ' . $options[$vars['batch_id']];
                                break;
                        }
                    $ret[] = ($vars['used'] ? 'Used ' : "Did't Use ") . $end;
                    break;
                default: 
                    return false;
            }
    ...
    function renderCommission()
    {
        ...
        foreach (array('first_payment' => 'First Payment', 'recurring' => 'Second and Subsequent Payments', 'free_signup' => 'Free Signup') as $fieldName => $label)
        {
...
class AffCommissionRuleTable extends Am_Table 
{
    ...
    public function processPayment(Invoice $invoice, InvoicePayment $payment = null)
    {
        ...
        for ($tier=1; $tier<=$this->getMaxTier(); $tier++) {
            ...
            if (!$aff_tier || !$aff_tier->is_affiliate)
                break;
