...
class Am_DbSync_Diff
{
    ...
    function addCreate($obj)  {  $this->addOp(self::CREATE, $obj);  }
    function addAlter($obj)   {  $this->addOp(self::ALTER, $obj);   }
    function addDrop($obj)    {  $this->addOp(self::DROP, $obj);    }
    function addDropConfirmed($obj) { $this->addOp(self::DROP_CONFIRMED, $obj); }
...
class Am_DbSync_Index extends Am_DbSync_Object
{
    ...
    static function createFromXml(SimpleXMLElement $index)
    {
        ...
        foreach ($index->field as $field) {
            $o = new stdclass;
            if ((string)$field['len']>0) $o->len = (string)$field['len'];
...
class Am_DbSync_Field extends Am_DbSync_Object {
    ...
    static function createFromDb(array $row)
    {
        ...
        if ($unsigned == 'unsigned') $f->unsigned = true;
        @list($f->type, $f->len) = preg_split('/[()]/', $type);
        if ($f->isInt()) $f->len = null;
    ...
    function getSqlDef()
    {
        ...
        if ($this->len) $ret.= "({$this->len})";
        if ($this->unsigned) $ret.= " unsigned";
        $ret .= " ";
        if (!$this->null)
            $ret .= "NOT NULL ";
...
class Am_DbSync_Table {
    ...
    static function createFromXml(SimpleXmlElement $table)
    {
        ...
        foreach ($table->alter as $alter)
            foreach ($alter->drop as $drop) {
                if ((string)$drop['field']) $t->addDropField((string)$drop['field']);
                if ((string)$drop['index']) $t->addDropIndex(((string)$drop['index']));
            }
        // check oldnames
    ...
    function getCreateSql($prefix)
    {
        ...
        $items = array();
        foreach ($this->getFields() as $field)  $items[] = "  " . $field->getSqlDef();
        foreach ($this->getIndexes() as $index) $items[] = "  " . $index->getSqlDef();
        $ret .= implode(",\n", $items) . "\n)";
...
class Am_DbSync
{
    ...
    {
        if ($utf8) $this->setCreateTableAdd (self::CREATE_UTF8);
    }
    function setCreateTableAdd($add)
    {
        $this->createTableAdd = $add;
    ...
    function getTables()  { return $this->tables; }
    function addDropTable($tableName)
    {
        if (!$tableName) throw new Exception("Empty tablename passed to " . __METHOD__);
    ...
    public function parseTables(DbSimple_Mysql $db)
    {
        ...
        {
            if (strlen($prefix) && (strpos($tablename, $prefix)!==0)) continue; // other prefix?
